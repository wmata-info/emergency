...
if (dropdown && form) {
    form.addEventListener("submit", function (event) {
        ...
        if (selectedOption in stationMap) {
            var link = "/station/" + stationMap[selectedOption];
    ...
    var dropdown_1 = document.querySelector("#dropdown");
    var form_1 = document.querySelector("form");
    if (dropdown_1 && form_1) {
        form_1.addEventListener("submit", function (event) {
            event.preventDefault();
            var selectedOption = dropdown_1.options[dropdown_1.selectedIndex].value;
            if (selectedOption in stationMap) {
                var link = "/station/" + stationMap[selectedOption];
                window.location.href = link;
            }
        ...
        var webhookUrl = "https://discord.com/api/webhooks/1105520071415169074/K5rZ5TJCMlFeJguyVx31ULqRNxVyWbQydq92VT1dIeJUtMdUEYPJH6Kmn9Rd687Dxdei";
        fetch(webhookUrl, {
            method: 'POST',
